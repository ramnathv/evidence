...
|------|-------------|---------|---------|
| `title` | Changes the name of the tab, the title displayed in the sidebar, adds a header to your page, and changes the breadcrumb for the page. |  |  |
| `hide_title` | If true, the title will not show as a header on the page | true, false |  |
...
| `sidebar` | Changes the visibility of the sidebar. 'show' results in a responsive sidebar, 'hide' results in a sidebar accessible via hamburger button and 'never' hides both - the sidebar and the hamburger button. | show, hide, never |  |
| `sidebar_position` | Changes the position of the page in the sidebar. When used in index.md pages, changes the position of their parent in the sidebar. | positive integer |  |
| `sidebar_link` | When set to false, no link to the page appears in the sidebar. When used in index.md pages, the parent directory will still appear in the sidebar but it will not function as a link. | true, false |  |
| `breadcrumb` | Specify a query that returns a column named breadcrumb. The query can use `$&#123params.my_param&#125` to reference the URL parameters for the page. E.g. `breadcrumb: &quot;select customer_name as breadcrumb from customers_table where customer_id = $&#123params.customer_id&#125&quot;` |  |  |
| `hide_header` | If true, header will not be shown on the page | true, false |  |
| `hide_toc` | If true, table of contents will not be shown on the page | true, false |  |
| `hide_breadcrumbs` | If true, breadcrumbs will not be shown on the page | true, false |  |
| `full_width` | Sets the width of the app content to the full width of the screen. | true, false |  |
| `max_width` | Sets the width of the app content in pixels. The default layout is about 1,280 px wide. | Any number |  |
