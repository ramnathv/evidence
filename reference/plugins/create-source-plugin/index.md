...
|------|-------------|---------|---------|
| `title` _(required)_ | Name or Title of the option |  |  |
| `type` _(required)_ | Control to show |  |  |
| `secret` | Secret values are placed in `connection.options.yaml`, which is not source controlled |  |  |
| `shown` | Displays value in UI elements (e.g. for usernames, that should not be source controlled but are not 'secret'. Otherwise the field will display as ∙∙∙) |  |  |
| `virtual` | Disables saving a field, useful for credential files |  |  |
| `references` | Indicates that the field should get its value from another field if it is available, useful for credential files. Formatted as a [json path](https://www.npmjs.com/package/@astronautlabs/jsonpath) |  |  |
| `forceReference` | If true, the input is disabled and the value can only come from a reference |  |  |
| `fileFormat` | If `type` is `file`, set how it should be parsed. It will then be available to `references` |  |  |
| `description` | Description of the option, shown as a hint in UI |  |  |
| `children` | See [children](#children) |  |  |
| `required` | Indicates that the user must provide this option |  |  |
| `options` | Available options for `select` type |  |  |
| `nest` | Determines behavior of `children` |  |  |
